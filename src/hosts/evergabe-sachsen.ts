...
export async function handleEvergabeSachsen(page: any, url: string): Promise<Map<string, Buffer>> {
    const documents = new Map<string, Buffer>();

    try {
        console.log('Detected Evergabe Sachsen - applying special handling...');
        ...
        for (let i = 0; i < 10; i++) {
            ...
            const hasExpectedContent = await page.evaluate(() => {
                ...
                return text.includes('Vergabeunterlagen') ||
        ...
        const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sachsen-docs-'));
        ...
        // Look for the download button in the Vergabeunterlagen table
        console.log('Looking for download button in Vergabeunterlagen table...');
        const downloadButton = await page.evaluate(() => {
            const tables = Array.from(document.querySelectorAll('table'));

            for (const table of tables) {
                const tableText = table.textContent || '';
                if (tableText.includes('Vergabeunterlagen') || tableText.includes('Version')) {
        ...
        if (downloadButton.found) {
            ...
            // Wait for popup to appear and handle the "Alles Auswählen" and "Auswahl herunterladen" flow
            ...
            for (let attempt = 0; attempt < 10 && !popupHandled; attempt++) {
                ...
                if (attempt === 0) {
                    const popupInfo = await page.evaluate(() => {
                        ...
                        return {
                            ...
                            visibleText: document.body.textContent?.includes('Vergabeunterlagen'),
                ...
                // First, try to click "Alles Auswählen" button
                const selectAllFound = await page.evaluate(() => {
                ...
                if (selectAllFound.found) {
                    console.log(`Found and clicked "Alles Auswählen" button: "${selectAllFound.text}"`);
                    await new Promise(resolve => setTimeout(resolve, 2000));
                }

                // Now look for "Auswahl herunterladen" button
        ...
        if (documents.size === 0) {
            ...
            console.log(`Successfully captured ${documents.size} document(s) from Evergabe Sachsen`);
        }

    } catch (error) {
        console.log('Special Evergabe Sachsen handling failed:', error);
    }
